...
rem =============================================================================
rem  WOS Japanese MOD - Windows (Knapford)
rem
rem  This .bat is rebuilt from the working .command flow:
rem    download repak -> migrate backup -> backup -> unpack -> overlay -> pack
rem    -> install -> cleanup
rem =============================================================================
...
rem Optional: manually specify the FULL PATH of TS2Prototype-WindowsNoEditor.pak
rem If set, Steam auto-detection is skipped.
set "GAME_PAK_OVERRIDE="

rem Backup location (v0.1.1+)
...
rem Work root
set "PACK_WORK_ROOT=%REPO_ROOT%WOS_pack_work_Knapford"
...
rem repak pack output pak path
...
rem MOD overlay dirs (same layout as .command)
set "MOD_OVERLAY_DIR=%REPO_ROOT%WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor"
set "MOD_OVERLAY_SODOR_CORE_DIR=%REPO_ROOT%WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor-Sodor-coredata"
set "MOD_OVERLAY_JAMES_CORE_DIR=%REPO_ROOT%WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor-James-coredata"
...
call :STEP_DOWNLOAD_REPAK || goto :FAILED
call :STEP_MIGRATE_V010_BACKUP || goto :FAILED
call :STEP_FIND_GAME_PAKS || goto :FAILED
call :STEP_BACKUP_ORIGINAL_PAKS || goto :FAILED
call :STEP_UNPACK_BACKUP_PAKS || goto :FAILED
call :STEP_OVERLAY_MOD_TO_TS2 || goto :FAILED
call :STEP_RUN_REPAK_PACK || goto :FAILED
call :STEP_VERIFY_OUTPUT || goto :FAILED
call :STEP_INSTALL_TO_GAME || goto :FAILED
call :STEP_CLEANUP_WORK || goto :FAILED
...
:STEP_DOWNLOAD_REPAK
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$r=Invoke-RestMethod -Uri $api -Headers @{'User-Agent'='wonders-of-sodor-mod/WOS_JapaneseMOD_Knapford.bat'};" ^
...
:STEP_MIGRATE_V010_BACKUP
  ...
  set "NEW_BACKUP_DIR=%ORIGINAL_PAK_DIR%"
  ...
  call :_MIGRATE_ONE "%OLD_BACKUP_DIR%\TS2Prototype-WindowsNoEditor.pak" "%NEW_BACKUP_DIR%\TS2Prototype-WindowsNoEditor.pak" || exit /b 1
  call :_MIGRATE_ONE "%OLD_BACKUP_DIR%\TS2Prototype-WindowsNoEditor-Sodor-coredata.pak" "%NEW_BACKUP_DIR%\TS2Prototype-WindowsNoEditor-Sodor-coredata.pak" || exit /b 1
  call :_MIGRATE_ONE "%OLD_BACKUP_DIR%\TS2Prototype-WindowsNoEditor-James-coredata.pak" "%NEW_BACKUP_DIR%\TS2Prototype-WindowsNoEditor-James-coredata.pak" || exit /b 1
...
:STEP_FIND_GAME_PAKS
  echo.
  echo [2/10] ゲーム pak の場所を解決 ...
  set "GAME_INSTALL_PAK="
  set "GAME_INSTALL_PAK_DIR="
  set "GAME_INSTALL_CORE_PAK="
  set "GAME_INSTALL_JAMES_CORE_PAK="

  if not "%GAME_PAK_OVERRIDE%"=="" (
    if exist "%GAME_PAK_OVERRIDE%" (
      set "GAME_INSTALL_PAK=%GAME_PAK_OVERRIDE%"
      goto :FOUND_GAME_PAKS
    ) else (
      echo [ERROR] GAME_PAK_OVERRIDE が存在しません: "%GAME_PAK_OVERRIDE%"
      exit /b 1
    )
  )
  ...
  for /f "usebackq delims=" %%P in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ErrorActionPreference='Stop';" ^
    "$rel='steamapps\common\Thomas & Friends™ Wonders of Sodor\WindowsNoEditor\TS2Prototype\Content\Paks\TS2Prototype-WindowsNoEditor.pak';" ^
    "$paths=@();" ^
    "$keys=@('HKCU:\Software\Valve\Steam','HKLM:\Software\WOW6432Node\Valve\Steam','HKLM:\Software\Valve\Steam');" ^
    "foreach($k in $keys){ try{ $p=(Get-ItemProperty -Path $k -Name SteamPath -ErrorAction Stop).SteamPath; if($p){ $paths += $p } } catch{} }" ^
    "$paths += 'C:\Program Files (x86)\Steam'; $paths += 'C:\Program Files\Steam';" ^
    "$libs=@();" ^
    "foreach($sp in ($paths | Where-Object { $_ -and (Test-Path -LiteralPath $_) } | Select-Object -Unique)){" ^
    "  $vdf=Join-Path $sp 'steamapps\libraryfolders.vdf';" ^
    "  if(Test-Path -LiteralPath $vdf){" ^
    "    $txt=Get-Content -LiteralPath $vdf -Raw;" ^
    "    $m=[regex]::Matches($txt,'\"path\"\\s*\"([^\"]+)\"');" ^
    "    foreach($mm in $m){ $libs += ($mm.Groups[1].Value -replace '\\\\','\') }" ^
    "  }" ^
    "}" ^
    "$libs += $paths;" ^
    "$cands=@();" ^
    "foreach($l in ($libs | Where-Object { $_ -and (Test-Path -LiteralPath $_) } | Select-Object -Unique)){" ^
    "  $cands += (Join-Path $l $rel);" ^
    "}" ^
    "foreach($p in $cands){ if(Test-Path -LiteralPath $p){ Write-Output $p; break } }" ^
  "`) do (
  ...
  if not defined GAME_INSTALL_PAK (
    echo [ERROR] ゲーム側の TS2Prototype-WindowsNoEditor.pak が見つかりません。
    echo        Steam のインストール先・ライブラリを確認してください。
    echo        見つからない場合は、この .bat の先頭で GAME_PAK_OVERRIDE にフルパスを設定してください。
    exit /b 1
  )

:FOUND_GAME_PAKS
  for %%I in ("%GAME_INSTALL_PAK%") do set "GAME_INSTALL_PAK_DIR=%%~dpI"
  ...
  if not exist "%GAME_INSTALL_CORE_PAK%" (
    echo [ERROR] ゲーム側の TS2Prototype-WindowsNoEditor-Sodor-coredata.pak が見つかりません: "%GAME_INSTALL_CORE_PAK%"
    exit /b 1
  )
  if not exist "%GAME_INSTALL_JAMES_CORE_PAK%" (
    echo [ERROR] ゲーム側の TS2Prototype-WindowsNoEditor-James-coredata.pak が見つかりません: "%GAME_INSTALL_JAMES_CORE_PAK%"
    exit /b 1
  )
...
:STEP_BACKUP_ORIGINAL_PAKS
  echo.
  echo [3/10] ゲーム元 pak のバックアップ (3種) ...
  call :_BACKUP_ONE "%GAME_INSTALL_PAK%" "%ORIGINAL_PAK%" || exit /b 1
  call :_BACKUP_ONE "%GAME_INSTALL_CORE_PAK%" "%ORIGINAL_SODOR_CORE_PAK%" || exit /b 1
  call :_BACKUP_ONE "%GAME_INSTALL_JAMES_CORE_PAK%" "%ORIGINAL_JAMES_CORE_PAK%" || exit /b 1
  exit /b 0
...
:_BACKUP_ONE
  ...
  if not exist "%ORIGINAL_PAK_DIR%\" mkdir "%ORIGINAL_PAK_DIR%" >nul 2>&1
  copy /Y "%SRC%" "%DST%" >nul
  if errorlevel 1 (
...
:STEP_UNPACK_BACKUP_PAKS
  echo.
  echo [4/10] バックアップ pak の展開 (3種) ^(repak unpack --output^) ...
  call :_UNPACK_ONE "%ORIGINAL_PAK%" "%UNPACK_OUTPUT_DIR%" "main pak" || exit /b 1
  call :_UNPACK_ONE "%ORIGINAL_SODOR_CORE_PAK%" "%UNPACK_CORE_OUTPUT_DIR%" "Sodor coredata" || exit /b 1
  call :_UNPACK_ONE "%ORIGINAL_JAMES_CORE_PAK%" "%UNPACK_JAMES_CORE_OUTPUT_DIR%" "James coredata" || exit /b 1
  exit /b 0
...
:_UNPACK_ONE
  ...
  rem repak が 0 以外を返すことがあるため、展開結果が存在すれば続行する
  set "HAS_OUTPUT="
  for /f "usebackq delims=" %%F in (`dir /a-d /s /b "%OUT_DIR%\*" 2^>nul`) do (
    set "HAS_OUTPUT=1"
    goto :_UNPACK_ONE_AFTER_OUTPUT_CHECK
  )
  :_UNPACK_ONE_AFTER_OUTPUT_CHECK
  if defined HAS_OUTPUT (
...
:STEP_OVERLAY_MOD_TO_TS2
  echo.
  echo [5/10] WOS_JapaneseMOD_Knapford を pak 展開先 (3種) に上書きコピー ...
  if not exist "%MOD_OVERLAY_DIR%\" (
    echo [ERROR] 差し替え元フォルダがありません: "%MOD_OVERLAY_DIR%"
    exit /b 1
  )
  if not exist "%UNPACK_OUTPUT_DIR%\" (
    echo [ERROR] unpack 済みの TS2Prototype-WindowsNoEditor がありません: "%UNPACK_OUTPUT_DIR%"
    exit /b 1
  )
  if not exist "%UNPACK_CORE_OUTPUT_DIR%\" (
    echo [ERROR] unpack 済みの TS2Prototype-WindowsNoEditor-Sodor-coredata がありません: "%UNPACK_CORE_OUTPUT_DIR%"
    exit /b 1
  )
  if not exist "%UNPACK_JAMES_CORE_OUTPUT_DIR%\" (
    echo [ERROR] unpack 済みの TS2Prototype-WindowsNoEditor-James-coredata がありません: "%UNPACK_JAMES_CORE_OUTPUT_DIR%"
    exit /b 1
  )
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ErrorActionPreference='Stop';" ^
    "$root=$env:UNPACK_CORE_OUTPUT_DIR;" ^
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$ErrorActionPreference='Stop';" ^
    "$root=$env:UNPACK_JAMES_CORE_OUTPUT_DIR;" ^
    "if (-not (Test-Path -LiteralPath $root)) { throw ('coredata dir not found: ' + $root) };" ^
    "$re='.*_S_(?:\d+|StoryName)_ja\.(?:uasset|uexp)$';" ^
    "$files=Get-ChildItem -LiteralPath $root -Recurse -File | Where-Object { $_.FullName -match $re };" ^
    "foreach($f in $files){ Remove-Item -LiteralPath $f.FullName -Force };" ^
  ...
  robocopy "%MOD_OVERLAY_DIR%" "%UNPACK_OUTPUT_DIR%" /E /IS /IT /R:2 /W:2 /NFL /NDL /NJH /NJS /NP
  if errorlevel 8 (
    echo [ERROR] 上書きコピー ^(robocopy^) に失敗しました。
    exit /b 1
  )
  if exist "%MOD_OVERLAY_SODOR_CORE_DIR%\" (
    robocopy "%MOD_OVERLAY_SODOR_CORE_DIR%" "%UNPACK_CORE_OUTPUT_DIR%" /E /IS /IT /R:2 /W:2 /NFL /NDL /NJH /NJS /NP
    if errorlevel 8 (
  ...
  if exist "%MOD_OVERLAY_JAMES_CORE_DIR%\" (
    robocopy "%MOD_OVERLAY_JAMES_CORE_DIR%" "%UNPACK_JAMES_CORE_OUTPUT_DIR%" /E /IS /IT /R:2 /W:2 /NFL /NDL /NJH /NJS /NP
    if errorlevel 8 (
  ...
  echo        反映先: "%UNPACK_OUTPUT_DIR%"
  echo        反映先: "%UNPACK_CORE_OUTPUT_DIR%"
  echo        反映先: "%UNPACK_JAMES_CORE_OUTPUT_DIR%"
  exit /b 0

:STEP_RUN_REPAK_PACK
  echo.
  echo [6/10] repak pack (3種) ^(--compression Zlib --version V11^) ...
  if not exist "%REPAK_EXE%" (
    echo [ERROR] repak が見つかりません: "%REPAK_EXE%"
    exit /b 1
  )
  if not exist "%UNPACK_OUTPUT_DIR%\" (
    echo [ERROR] パック対象がありません: "%UNPACK_OUTPUT_DIR%"
    exit /b 1
  )
  if not exist "%UNPACK_CORE_OUTPUT_DIR%\" (
    echo [ERROR] パック対象 (coredata) がありません: "%UNPACK_CORE_OUTPUT_DIR%"
    exit /b 1
  )
  if not exist "%UNPACK_JAMES_CORE_OUTPUT_DIR%\" (
    echo [ERROR] パック対象 (James coredata) がありません: "%UNPACK_JAMES_CORE_OUTPUT_DIR%"
    exit /b 1
  )
  "%REPAK_EXE%" pack --compression Zlib --version V11 "%UNPACK_OUTPUT_DIR%" "%OUTPUT_PAK%"
  if errorlevel 1 (
    echo [ERROR] repak pack に失敗しました。
    exit /b 1
  )
  "%REPAK_EXE%" pack --compression Zlib --version V11 "%UNPACK_CORE_OUTPUT_DIR%" "%OUTPUT_CORE_PAK%"
  if errorlevel 1 (
    echo [ERROR] repak pack (coredata) に失敗しました。
    exit /b 1
  )
  "%REPAK_EXE%" pack --compression Zlib --version V11 "%UNPACK_JAMES_CORE_OUTPUT_DIR%" "%OUTPUT_JAMES_CORE_PAK%"
  if errorlevel 1 (
