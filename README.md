...
ゲームの pak（`TS2Prototype-WindowsNoEditor-*.pak`）を **repak** で展開→MOD差し込み→再パックし、ゲームの `Paks` フォルダへ適用します（初回はバックアップを作成します）。

**※ 現時点では翻訳の修正は一部のみ、試験的実装です。今後のアップデートで順次修正範囲を拡大予定です。**
...
**開発者向け:** `v` で始まるタグ（例: `v0.2.1`）をリモートに push すると、GitHub Actions がビルドして **GitHub Release に zip を自動アップロード**します（手動のワークフロー実行でも同様に公開できます）。
