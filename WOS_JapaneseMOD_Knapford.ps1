...
Write-Host ""
Write-Host "=== WOS_JapaneseMOD (Windows / Knapford) ==="
...
$PackWorkRoot = Join-Path $RepoRoot 'WOS_pack_work_Knapford'
...
$ModOverlayDir      = Join-Path $RepoRoot 'WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor'
$ModOverlayCoreDir  = Join-Path $RepoRoot 'WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor-Sodor-coredata'
$ModOverlayJamesDir = Join-Path $RepoRoot 'WOS_JapaneseMOD_Knapford\TS2Prototype-WindowsNoEditor-James-coredata'
...
function Download-Repak() {
  ...
  # If nested directory exists, flatten it.
...
function Unpack-One([string]$pak, [string]$outDir) {
  ...
  $rc = $LASTEXITCODE
  if ($rc -ne 0) {
    # Accept non-zero when output exists (same approach as bat)
...
function Overlay-Mod() {
  Write-Host ""
  Write-Host "[4/10] WOS_JapaneseMOD_Knapford を pak 展開先 (3種) に上書きコピー ..."
...
try {
  ...
  # steps 5-7 are currently no-ops in .bat/.command
  Write-Host ""
